...
const CommentService = {
    ...
    setDBNewComment:({userId, commentText})=>{
        db.query('INSERT INTO comment (userId, commentText) VALUES(?, ?)', [userId, commentText],
    (err, result) => {
        if (err) {
    ...
    updateDBComment:({id, comment_text,}) =>{
        db.query('UPDATE comment SET comment_text =? WHERE id =?', [comment_text, id],
        (err, result) => {
            if (err) {
